...
export const Label_Informacao = styled.p`
    background: linear-gradient(
        180deg,
        rgba(226, 226, 226, 0.83) 0%,
        rgba(214, 214, 214, 0.83) 52.63%,
        rgba(115, 115, 115, 0.83) 100%
    );
...
async function buscaEstatisticas(setState, partida_id) {
    const data = await axiosInstance
        .get(`/match/${partida_id}/statistics`)
        .then((res) => {
            return res.data;
        });

    if (data.length === 0) return setState(null);
    ...
    if (
        parseInt(dadosOrdenados[length - 1].partidaId) === parseInt(partida_id)
    ) {
        await setState(
            `${formatarTempo(dadosOrdenados[length - 1].createdAt)} - ${
                dadosOrdenados[length - 1].categoria.nome
            } de ${dadosOrdenados[length - 1].jogador.nome}`
        );
    } else {
        setState(null);
    }
...
export default function Botao_Informativo() {
    ...
    const [ultimaEstatistica, setUltimaEstatistica] =
        useRecoilState(ultima_estatistica);
    ...
    return (
        <Container_Botao_Sorteio img_url={imagem_botao.src}>
            <Label_Informacao className={poppins.className}>
                {ultimaEstatistica}
            </Label_Informacao>
